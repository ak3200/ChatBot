...
def smart_chunk_text(text, chunk_size=500, overlap=0):
    ...
    if current_chunk:
        chunks.append(current_chunk)
    # Overlap logic (optional, not used here for sentence-level)
    # For overlap, you can combine sentences with previous/next as needed
    return chunks
...
def retrieve_relevant_chunks(question, session_id, top_k=15):
    ...
    # Get selected model for session
    model_filter = session_model_context.get(session_id, None)
    ...
    for i in indices[0]:
        if 0 <= i < len(chunk_texts):
            ...
            if model_filter and model_filter.lower() not in chunk.lower():
                continue
            if len(chunk) > 50:
                results.append({
                    "chunk": chunk,
...
Rules:
- If the answer is explicitly stated in the context, respond with it directly. DO NOT say "The dataset explicitly mentions that..."
...
- Filter or reject answers if confidence is low.
- If the answer is inferred, clearly state it is inferred and not explicitly confirmed.
- Mention if the dataset does not explicitly confirm the answer.
...
async def ask_question(payload: QueryPayload):
    ...
    results = retrieve_relevant_chunks(question, session_id)
